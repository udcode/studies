/*
File: main.cpp
Description: This is the test program (main) for
...
int main() {
    ...
    for(int i =0;i<SIZE_ARR;++i){
        arrInteger[i]=(-10)+(rand()%(21));
        //fill with float random number
        arrFloat[i]=(-10.0f)+static_cast<float>(rand())/( static_cast<float>(RAND_MAX/(20.0f)));
    ...
    do {
        ...
        case PRINT_ARRAY:
            ...
            case RATIONAL_ARRAY:
                ...
                for (int i = 0; i < SIZE_ARR; ++i)
                    cout << *arrRational[i]<<endl;
                break;
            case COMPLEX_ARRAY:
                //print the complex array
                for(int i=0;i<SIZE_ARR;++i)
                    cout<<*arrComplex[i]<<endl;
                break;
            default:
                cout <<"ERROR: enter a valid array number"<<endl;
                break;
        ...
        case SUB_TWO_ELEMENTS:
            ...
            if (src1 >= 0 && src1 <= 3 && src2 >= 0 && src2 <= 3 && dest >= 0 && dest <= 3){//check for bounds
                ...
                case RATIONAL_ARRAY:
                    ...
                    operation(arrRational[dest], arrRational[src1], arrRational[src2], subRational);
                    break;
                case COMPLEX_ARRAY:
                    //sub two element to another in complex array
                    operation(arrComplex[dest], arrComplex[src1], arrComplex[src2], subComplex);
                    break;
                default:
                    cout <<"ERROR: enter a valid array number"<<endl;
                    break;
        ...
        case SORT_ARRAY:
            ...
            case INT_ARRAY:
                //sort the int array
                qsort(arrInteger, SIZE_ARR, sizeof(int), [](const void* a, const void* b) { return (*(int*)a - *(int*)b); });
                break;
            case FLOAT_ARRAY:
                //sort the float array
                qsort(arrFloat, SIZE_ARR, sizeof(float), [](const void* a, const void* b) {  if (*(float*)a == *(float*)b)return 0; else if (*(float*)a > *(float*)b)return 1; else return -1; });
                break;
            case RATIONAL_ARRAY:
                //sort the rational array
                qsort(arrRational,SIZE_ARR,sizeof(Rational*),compareRational);
                break;
            case COMPLEX_ARRAY:
                //sort the complex array
                qsort(arrComplex,SIZE_ARR,sizeof(Complex *),compareComplex);
                break;
            default:
                cout <<"ERROR: enter a valid array number"<<endl;
                break;
