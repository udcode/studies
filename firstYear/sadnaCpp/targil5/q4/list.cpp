...
List merge(List& lst1, List& lst2){
    List::Node * lst1Prev = lst1.head;
    List::Node * lst2Prev = lst2.head;
    //check for empty lists
    if(lst1Prev==nullptr)return lst2;
    if(lst2Prev==nullptr)return lst1;

    List tmp;
    List::Node * tmpPrev;//pointer to last inserted item
    //insert the first element. it has to be outside the loop, for that the head has no next
    //copy the items by their values order
    if(lst1Prev->value()<=lst2Prev->value()){
        tmp.head=new List::Node (lst1Prev->value(),nullptr);//insert
        lst1Prev=lst1Prev->next();//move forword
    }else{
        tmp.head=new List::Node (lst2Prev->value(),nullptr);//insert
        lst2Prev=lst2Prev->next();//move forword
    }
    tmpPrev=tmp.head;//get forword
    //copy every item from the lists by their order
    //until one of the list rich the end
    while(lst1Prev!=nullptr&&lst2Prev!=nullptr){
        if(lst1Prev->value()<=lst2Prev->value()){
            tmpPrev->next(new List::Node (lst1Prev->value(),nullptr));//insert
            lst1Prev=lst1Prev->next();//move forword
        }else{
            tmpPrev->next(new List::Node (lst2Prev->value(),nullptr));//insert
            lst2Prev=lst2Prev->next();//move forword
        }
        tmpPrev=tmpPrev->next();//move forword
    }
    while(lst1Prev!=nullptr){//copy the rest of the list
        tmpPrev->next(new List::Node (lst1Prev->value(),nullptr));//insert
        tmpPrev=tmpPrev->next();//move forword
        lst1Prev=lst1Prev->next();//move forword
    }
    while(lst2Prev!=nullptr){//copy the rest of the list
        tmpPrev->next(new List::Node (lst2Prev->value(),nullptr));//insert
        tmpPrev=tmpPrev->next();//move forword
        lst2Prev=lst2Prev->next();//move forword
    }
...
void makeSet(List & list){
	...
	List::Node * corrent = list.head;//setting a pointer to the corrent node
	List::Node* nextNode = corrent->next();//setting a pointer to the next node

	while (nextNode != nullptr)//run untill the end of the list
	{
        if (corrent->value() == nextNode->value()){//if there are duplecated values
         //---------not using the remove function that will cost in worst case overall of O(n^2):kipping it on O(n)
			corrent->next(nextNode->next());//skipping one Node forward
			delete nextNode;//deleting the redundant value
			nextNode = corrent->next();//setting the nextNode to the next node
			
        }else{ //if the values are not the same just move the pointer foreward
			corrent = nextNode;
